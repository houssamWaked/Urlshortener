...
class UserRepository {
    ...
    static async createUser(userData) {
        try {
            ...
            if (emailExists) {
                throw new Error(`User with email ${userData.user_email} already exists`);
            }
    ...
    static async updateUser(user_id, userData) {
        try {
            ...
            if (userData.user_email) {
                const emailExists = await this.userExistsByEmail(userData.user_email, user_id);
                if (emailExists) throw new Error(`User with email ${userData.user_email} already exists`);
    ...
    static async userExistsByEmail(user_email, excludeUserId = null) {
        try {
            const where = excludeUserId
                ? { user_email, user_id: { [require('sequelize').Op.ne]: excludeUserId } }
                : { user_email };
