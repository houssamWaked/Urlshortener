...
class urlRepository {
    ...
    static async createUrl(urlData) {
        try {
            ...
            if (!urlData.long_url || !urlData.short_code) {
                throw new Error('Both `long_url` and `short_code` are required.');
            }
            ...
            const existingUrl = await urls.findOne({ where: { short_code: urlData.short_code } });
            if (existingUrl) {
    ...
    // Helper: Validate short code (alphanumeric, 1-10 chars)
    ...
    static async redirectToOriginalUrl(short_code) {
        try {
            if (!short_code || typeof short_code !== 'string') {
                throw new Error('Valid `short_code` (string) is required.');
            }
